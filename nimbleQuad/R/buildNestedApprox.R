...
buildNestedApprox <- nimbleFunction(
    ...
    setup = function(model, paramNodes, latentNodes, calcNodes, calcNodesOther, control = list()) {
        ...
        ## Default to CCD
        paramGrid <- configureQuadGrid(d = 1, nQuad_ = nQuadOuter, quadRule = paramGridRule,
        ...
        paramMargGrid <- configureQuadGrid(d = nParamTrans - 1, nQuad_ = 1,
    ...
    methods = list(
        ...
        buildParamGrid = function(quadRule = character(0, default = "NULL"),
            ...
            paramGrid$buildGrid(method = paramGridRule, nQuad = nQuadOuter)
        ...
        calcParamGrid = function(skew = logical(0, default = TRUE)) {
            ...
            for (i in 1:nGrid) {
                ...
                if (i == paramGrid$modeI()) {
                    wgt <- paramGrid$weights(indx = i)[1]
        ...
        calcMarginalParamQuad = function(pIndex = integer(),
            ...
            if (dim(paramTrans1_nodes)[1] != nPts) paramTrans1_nodes <<- AGHQ1D(nQuad = nPts)
            ...
            paramMargGrid$buildGrid(nQuad = nQuad)  
            ...
            res <- matrix(0, nrow = nPts, ncol = 2)
            paramTransj <- paramTransMode
            other_paramTrans_indices <- paramTrans_indices[paramTrans_indices != pIndex]

            ## For each value of paramTransi, we need to do AGHQ which means finding the
            ...
            for (i in 1:nPts) {
                ...
                for (j in 1:nQuadGrid) {
                    nimCat(".")
                    if (j != paramMargGrid$modeI()) {
                        nodej <- paramMargGrid$nodes(indx = j)[1, ]
                        paramTrans_tmp <- z_to_paramTrans(z = nodej, postMode = paramTrans_iMode, A = Atransform_i,
                                                method = gridTransformMethod)
                        paramTransj[other_paramTrans_indices] <- paramTrans_tmp
                        postLogDensij <- innerMethods$calcLogDens_pTransformed(pTransform = paramTransj)
                        density_i <- density_i + exp(postLogDensij - maxLogDensity_i) * paramMargGrid$weights(indx = j)[1]
                    } else {
                        density_i <- density_i + paramMargGrid$weights(indx = j)[1]
                    }
                }
                res[i, 2] <- log(density_i) + maxLogDensity_i - 0.5 * logDetNegHessParamTrans_i
            }
            nimCat("\n")
            ## Because paramTransi values are AGHQ, we can normalize to get the proper
            ...
            ## normalizing P(paramTransi,Y) to get P(Y) rather than the expensive
        ...
        calcMarginalParamIntegFree = function(pIndex = integer()) {
            ...
            stdDev <- sqrt(covParamTrans[pIndex, pIndex])
            paramTransi <- numeric(value = 0, length = nParamTrans)
            setTransformations(transformMethod)
            for (i in 1:nzMargGrid) {
                                        # Known fixed # of points
                paramTransi[pIndex] <- paramTransMode[pIndex] + zMargGrid[i] * stdDev
                marg_paramTrans[pIndex, i, 1] <<- paramTransi[pIndex]
                ## Find the conditional mean:
                for (j in 1:nParamTrans) {
                    if (j != pIndex) {
                        paramTransi[j] <- paramTransMode[j] + covParamTrans[pIndex, j] / covParamTrans[pIndex, pIndex] *
                            (paramTransi[pIndex] - paramTransMode[pIndex])
                    }
                }
                ## Calculate asymmetric Gaussian:
                zi <- paramTrans_to_z(paramTransi, paramTransMode, AinverseTransform, transformMethod)
