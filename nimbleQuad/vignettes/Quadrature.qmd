...
2. Deterministic nested quadrature methods for posterior approximation using Laplace approximation for the inner marginalization of the latent nodes and general quadrature methods for outer marginalization of the parameter nodes. These methods borrow ideas from the popular INLA approach and from the extended Gaussian latent model approach provided in the R `aghq` package.
...
If not trying to use the same model as INLA, one might choose different priors and would not necessarily need to define the logit-normal distribution.
...
If one wishes to find the posterior mode, one can use `findMAP`.
...
4. Simulate the joint posterior of the `latentNodes` from a mixture of multivariate normal distributions with mean, covariance, and weights obtained at each grid point in the parameter quadrature grid.
...
legend("topright", legend = c("NIMBLE nested approx", "INLA", "HMC"),
  col = c('black','red','green'), lty = rep(1,3), cex = 0.75)
...
capprox$buildParamGrid(quadRule = "AGHQ", nQuadUpdate = 7, prune = 0.5)
